...
from registry import ArtifactManager, ArtifactData
from registry import ArtifactCache
...
class MLflowRegistry(ArtifactManager):
    ...
    >>> from numalogic.test_registry import MLflowRegistry
    >>> from sklearn.preprocessing import StandardScaler
    >>>
    >>> data = [[0, 0], [0, 0], [1, 1], [1, 1]]
    >>> scaler = StandardScaler.fit(data)
    >>> test_registry = MLflowRegistry(tracking_uri="http://0.0.0.0:8080")
    >>> test_registry.save(skeys=["model"], dkeys=["AE"], artifact=VanillaAE(10))
    >>> artifact_data = test_registry.load(skeys=["model"], dkeys=["AE"], artifact_type="pytorch")
    """
    ...
    def save(
        ...
        **metadata: META_VT,
        ...
        """Saves the artifact into mlflow test_registry and updates version.
    ...
    def delete(self, skeys: KEYS, dkeys: KEYS, version: str) -> None:
        """Deletes the artifact with a specified version from mlflow test_registry.

...
from nlregistry.artifact import ArtifactData
from nlregistry.localcache import LocalLRUCache
from nlregistry.mlflow_registry import ModelStage, MLflowRegistry
from numalogic.tools.exceptions import ModelVersionError
from tests._mlflow_utils import (
    model_sklearn,
    create_model,
...
class TestMLflow(unittest.TestCase):
    ...
    @patch(
        ...
        Mock(return_value=return_pytorch_rundata_dict()),
    )
    ...
    @patch(
        "mlflow.tracking.MlflowClient.get_run",
        Mock(return_value=return_empty_rundata()),
    )

...
from nlregistry.artifact import (
    ArtifactManager,
    ArtifactData,
    ArtifactCache,
    _apply_jitter,
)
from nlregistry._serialize import loads, dumps
from numalogic.tools.exceptions import ModelKeyNotFound, RedisRegistryError
from numalogic.tools.types import (
    artifact_t,
    redis_client_t,
    KEYS,
    META_T,
...
class RedisRegistry(ArtifactManager):
    ...
    ----
        ...
        cache_registry: Cache test_registry to use (default = None).
        transactional: Flag to indicate if the test_registry should be transactional or
        not (default = False).
    ...
    def __load_latest_artifact(self, key: str) -> tuple[ArtifactData, bool]:
        """
        Load the latest artifact from the test_registry.
        ...
        Raises
        ------
            ModelKeyNotFound: If the model key is not found in the test_registry.
        """
            ...
            if not is_cached:
                if latest:
                    _LOGGER.debug(
                        ...
                        self.__construct_latest_key(key),
                        key,
                    )
    ...
    def save(
        ...
        **metadata: META_VT,
        ...
        """Saves the artifact into redis test_registry and updates version.
    ...
    def delete(self, skeys: KEYS, dkeys: KEYS, version: str) -> None:
        """Deletes the model version from test_registry.
        ...
        Raises
        ------
            ModelKeyNotFound: If the model version is not found in test_registry.
        ...
        try:
            with self.client.pipeline(transaction=self.transactional) as pipe:
                ...
                for _, value in dict_artifacts.items():
                    ...
                    version_key = self.client.get(name=latest_key)
                    pipe.hset(
                        name=version_key.decode(),
                        key="metadata",
                        value=orjson.dumps(metadata),
                    )
                pipe.execute()
        except RedisError as err:
            raise RedisRegistryError(f"{err.__class__.__name__} raised") from err
    ...
    def save_multiple(
        ...
        **metadata: META_VT,
        ...
        """
        Saves multiple artifacts into redis test_registry. The last save stores all the

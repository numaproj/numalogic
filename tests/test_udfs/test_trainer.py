...
from connectors import RedisConf, DruidConf, DruidFetcherConf, PrometheusFetcher
from connectors import DruidFetcher
...
class TestDruidTrainerUDF(unittest.TestCase):
    def setUp(self):
        ...
        conf_1 = OmegaConf.load(os.path.join(TESTS_DIR, "test_udfs", "resources", "_config.yaml"))
    ...
    def test_druid_from_config_1(self):
        with self.assertLogs(level="WARN") as log:
            self.udf1(self.keys, self.datum)
            self.assertLogs(
                "WARNING:numalogic.test_udfs._logger:uuid='some-uuid' event='Caught "
...
class TestPrometheusTrainerUDF(unittest.TestCase):
    def setUp(self):
        ...
        conf = OmegaConf.load(os.path.join(TESTS_DIR, "test_udfs", "resources", "_config3.yaml"))

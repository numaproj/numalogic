...
DATUM = input_json_from_file(
    os.path.join(TESTS_DIR, "test_udfs", "resources", "data", "stream.json")
)
...
def setup():
    ...
    _given_conf = OmegaConf.load(os.path.join(TESTS_DIR, "test_udfs", "resources", "_config.yaml"))
    _given_conf_2 = OmegaConf.load(
        os.path.join(TESTS_DIR, "test_udfs", "resources", "_config2.yaml")
    )
...
def test_preprocess_data_error(setup):
    udf1, _ = setup
    with open(os.path.join(TESTS_DIR, "test_udfs", "resources", "data", "stream.json"), "rb") as f:
        stream = json.load(f)

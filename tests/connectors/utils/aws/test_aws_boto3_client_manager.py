...
class TestBoto3ClientManager:
    ...
    def test_init(self):
        assert isinstance(self.boto3_client_manager.sts_client_manager, STSClientManager)

    def test_get_boto3_session(self):
        self.boto3_client_manager.sts_client_manager.get_credentials = MagicMock(
            return_value={
        ...
        with patch.object(
            Boto3ClientManager, "get_boto3_session", return_value=self.boto3_session_mock
        ) as boto3_session_class:
            boto3_session = self.boto3_client_manager.get_boto3_session()

            assert boto3_session == self.boto3_session_mock
    ...
    def test_get_client_rds(self):
        self.boto3_client_manager.get_boto3_session = MagicMock(
            return_value=self.boto3_session_mock
        )
        ...
        self.boto3_session_mock.client.assert_called_with(
            "rds", region_name=self.boto3_client_manager.configurations.aws_region
        )
        assert rds_client == self.rds_client_mock
    ...
    def test_get_client_athena(self):
        ...
        self.boto3_client_manager.get_client("athena")

        self.boto3_client_manager.get_boto3_session.assert_called_once()
        self.boto3_session_mock.client.assert_called_with(
            "athena", region_name=self.boto3_client_manager.configurations.aws_region
        )

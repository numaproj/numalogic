...
class TestAwsConfig(unittest.TestCase):
    def test_load_db_conf_file_exists(self):
        result = db_configuration.load_db_conf(os.path.join(TESTS_DIR,
                                                            "resources", "rds_db_config.yaml"))
        # here add the asserts to validate that the config has been created as expected
        self.assertIsNotNone(result)

    @patch("numalogic.connectors.utils.aws.db_configurations.OmegaConf.load",

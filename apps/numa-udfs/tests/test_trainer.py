...
from nlconnectors.config import RedisConf, DruidConf, DruidFetcherConf
from nlconnectors import DruidFetcher, PrometheusFetcher
from numalogic.tools.exceptions import (
    ConfigNotFoundError,
    DruidFetcherError,
    PrometheusFetcherError,
)
from src import StreamConf, PipelineConf, MLPipelineConf
from src.tools import TrainMsgDeduplicator
from src.trainer import DruidTrainerUDF, PromTrainerUDF
...
class TestDruidTrainerUDF(unittest.TestCase):
    def setUp(self):
        ...
        conf_1 = OmegaConf.load(os.path.join(TESTS_DIR, "resources", "_config.yaml"))
    ...
    def test_druid_from_config_1(self):
        with self.assertLogs(level="WARN") as log:
            self.udf1(self.keys, self.datum)
            self.assertLogs(
                "WARNING:numalogic.tests._logger:uuid='some-uuid' event='Caught "
...
class TestPrometheusTrainerUDF(unittest.TestCase):
    def setUp(self):
        ...
        conf = OmegaConf.load(os.path.join(TESTS_DIR, "resources", "_config3.yaml"))

...
from src import StreamConf, InferenceUDF, MLPipelineConf
from src.entities import StreamPayload, Header, Status, TrainerPayload
...
def test_model_pass_error_02(udf_with_adjust, udf_args, mocker):
    mocker.patch.object(
        ...
        "load",
        return_value=ArtifactData(
            artifact=VanillaAE(seq_len=15, n_features=1),
            extras=dict(version="0", timestamp=time.time(), source="test_registry"),
            metadata={},
        ),
    )
